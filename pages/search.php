...
$sort = sanitize($_GET['sort'] ?? 'title_asc');
...
<div class="search-hero">
    ...
    <p>Search across our full catalog — books, supplies, and more</p>
    ...
    <form method="GET" action="">
        <div class="search-bar-wrap">
            ...
            <?php if($sort !== 'title_asc'): ?><input type="hidden" name="sort" value="<?= htmlspecialchars($sort) ?>"><?php endif; ?>
        ...
        <div class="category-pills">
            <a href="<?= buildPillUrl('', $query, $priceMin, $priceMax, $availability, $sort) ?>" class="pill <?= !$category ? 'active' : '' ?>">All</a>
            <a href="<?= buildPillUrl('textbook', $query, $priceMin, $priceMax, $availability, $sort) ?>" class="pill <?= $category === 'textbook' ? 'active' : '' ?>">
            ...
            <a href="<?= buildPillUrl('office_supply', $query, $priceMin, $priceMax, $availability, $sort) ?>" class="pill <?= $category === 'office_supply' ? 'active' : '' ?>">
    ...
    <div class="search-layout">
        ...
        <aside class="search-sidebar">
            ...
            <div class="sidebar-section">
                ...
                <div class="price-row">
                    <input type="number" name="price_min" class="price-input" placeholder="Min" min="0" step="0.01"
                        value="<?= $priceMin !== null ? htmlspecialchars($priceMin) : '' ?>">
                    <span class="price-dash">&ndash;</span>
                    <input type="number" name="price_max" class="price-input" placeholder="Max" min="0" step="0.01"
            ...
            <div class="sidebar-section">
                ...
                <label class="filter-option">
                    <input type="radio" name="availability" value="out_of_stock" <?= $availability === 'out_of_stock' ? 'checked' : '' ?>>
            ...
            <div class="sidebar-section">
                <div class="sidebar-label">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="14" y2="12"/><line x1="4" y1="18" x2="8" y2="18"/></svg>
                    Sort
                </div>
                <label class="filter-option">
                    <input type="radio" name="sort" value="title_asc" <?= $sort === 'title_asc' ? 'checked' : '' ?>>
                    <span class="filter-option-label">Title A–Z</span>
                </label>
                <label class="filter-option">
                    <input type="radio" name="sort" value="title_desc" <?= $sort === 'title_desc' ? 'checked' : '' ?>>
                    <span class="filter-option-label">Title Z–A</span>
                </label>
                <label class="filter-option">
                ...
                <label class="filter-option">
                    <input type="radio" name="sort" value="newest" <?= $sort === 'newest' ? 'checked' : '' ?>>
            ...
            <div class="sidebar-apply">
                ...
                <a href="search.php<?= $query ? '?q=' . urlencode($query) : '' ?>" class="btn-clear">Clear all filters</a>
            </div>
        ...
        <div class="results-area">
            ...
            <?php if ($hasSearched): ?>
                <p class="results-meta">
                    ...
                    <?php if($query): ?> for &ldquo;<?= htmlspecialchars($query) ?>&rdquo;<?php endif; ?>
                    <?php if($category): ?> in <strong><?= $category === 'office_supply' ? 'Office Supplies' : 'Textbooks' ?></strong><?php endif; ?>
                </p>
            <?php endif; ?>
            ...
            <div class="empty-state">
                <div class="empty-state-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div>
