...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Always re-fetch current status so we know what's allowed
    ...
    if (in_array($action, ['item_update', 'item_remove'], true)) {
        ...
        // If no items left, auto-cancel (stock was already restored per-remove)
...
// Load order items joined with books so we know current available stock
$items = [];
$itemsRes = $conn->query("
...
<div class="page-header">
    ...
    <p>Placed on <?= date('F j, Y \a\t g:i A', strtotime($order['created_at'])) ?>
       &nbsp;·&nbsp;
       Last updated <?= date('M j, g:i A', strtotime($order['updated_at'])) ?></p>
</div>
    ...
    <div class="admin-info-grid">
        <div class="admin-info-card">
            ...
            <div class="admin-info-card-body">
                <div class="admin-info-row"><strong><?= htmlspecialchars(trim(($order['user_first'] ?? '') . ' ' . ($order['user_last'] ?? ''))) ?></strong></div>
                <div class="admin-info-row"><a href="mailto:<?= htmlspecialchars($order['user_email'] ?? '') ?>" class="admin-info-link"><?= htmlspecialchars($order['user_email'] ?? '—') ?></a></div>
                <?php if(!empty($order['user_student_id'])): ?>
                <div class="admin-info-row admin-info-sub">Student ID: <?= htmlspecialchars($order['user_student_id']) ?></div>
                <?php endif; ?>
            </div>
        </div>
        <div class="admin-info-card">
            ...
            <div class="admin-info-card-body">
                <div class="admin-info-row"><strong><?= htmlspecialchars($order['full_name']) ?></strong></div>
                <div class="admin-info-row">
                    <?= htmlspecialchars($order['street']) ?>
                    <?php if(!empty($order['apt'])): ?>, <?= htmlspecialchars($order['apt']) ?><?php endif; ?>
                </div>
                <div class="admin-info-row"><?= htmlspecialchars($order['city']) ?>, <?= htmlspecialchars($order['state']) ?> <?= htmlspecialchars($order['zip']) ?></div>
                <?php if(!empty($order['phone'])): ?>
                <div class="admin-info-row admin-info-sub">Phone: <?= htmlspecialchars($order['phone']) ?></div>
                <?php endif; ?>
            </div>
        </div>
        <div class="admin-info-card">
            ...
            <div class="admin-info-card-body">
                <div class="admin-info-row admin-info-row-split"><span>Subtotal</span><span>$<?= number_format($order['subtotal'], 2) ?></span></div>
                <div class="admin-info-row admin-info-row-split"><span>Shipping</span>
                    <span><?php if($order['shipping_cost'] == 0): ?><span class="free-shipping">Free</span><?php else: ?>$<?= number_format($order['shipping_cost'], 2) ?><?php endif; ?></span>
                </div>
                <div class="admin-info-row admin-info-row-split"><span>Tax</span><span>$<?= number_format($order['tax'], 2) ?></span></div>
                <hr class="admin-info-divider">
                <div class="admin-info-row admin-info-row-split admin-info-row-total"><span>Total</span><span>$<?= number_format($order['total'], 2) ?></span></div>
            </div>
        </div>
    </div>
    ...
    <div class="cart-panel" style="margin-top:24px;">
        <div class="cart-panel-header">
            ...
            <span class="cart-panel-title" style="font-weight:500; text-transform:none; letter-spacing:0;">
                ...
                <?php if(!$canEditItems): ?>
                <span style="color:var(--ink-faint); margin-left:8px;">· editing locked (status is <?= $badgeLabel ?>)</span>
                <?php endif; ?>
            </span>
        </div>
        ...
        <div class="cart-item">
            ...
            <div class="cart-item-info">
                ...
                <?php else: ?>
                <div class="cart-item-author" style="font-size:11px; color:#c0392b; margin-top:2px;">
                    ...
                    <div class="qty-controls">
                        ...
                        <form method="POST" action="" class="qty-form">
                            ...
                            <?php if($item['quantity'] <= 1): ?>
                                ...
                                <button type="submit" class="qty-btn" title="Remove item">−</button>
                            <?php else: ?>
            ...
            <?php if($canEditItems): ?>
            <form method="POST" action="" onsubmit="return confirm('Remove this item from the order? Stock will be restored.');">
                <input type="hidden" name="action" value="item_remove">
                <input type="hidden" name="item_id" value="<?= $item['id'] ?>">
                <button type="submit" class="btn-remove" title="Remove item from order">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>
                </button>
            </form>
            <?php endif; ?>
        </div>
