...
// Built by Alok and Kelsang 
// Kushal: cart count query added later when checkout was built 

require_once __DIR__ . '/config.php';

// Kushal: get cart count for the badge in header
$cartCount = 0;
$conn = getConnection();
if (isLoggedIn()) {
...
// Alok: track current page for active nav link highlight
...
<head>
    ...
    <!-- Kelsang: google fonts for Instrument Serif + DM Sans -->
...
<body>
    ...
    <header class="site-header">
        <div class="header-inner">
            ...
            <!-- Logo -->
            <!-- Alok: links back to home from any page -->
            ...
            <!-- Main nav -->
            <!-- Alok: active class added to current page link -->
            <nav class="main-nav">
                <a href="<?= SITE_URL ?>/index.php"
                   class="<?= $currentPage === 'index.php' ? 'active' : '' ?>">Home</a>
                <a href="<?= SITE_URL ?>/pages/books.php"
                   class="<?= $currentPage === 'books.php' ? 'active' : '' ?>">Products</a>
                <a href="<?= SITE_URL ?>/pages/search.php"
                   class="<?= $currentPage === 'search.php' ? 'active' : '' ?>">Search</a>

                <?php if (isLoggedIn()): ?>
                    <a href="<?= SITE_URL ?>/pages/orders.php"
                       class="<?= $currentPage === 'orders.php' ? 'active' : '' ?>">Orders</a>
                <?php endif; ?>

                <!-- Kushal: admin link only show if role is admin -->
                <?php if (isLoggedIn() && ($_SESSION['role'] ?? '') === 'admin'): ?>
                    <a href="<?= SITE_URL ?>/pages/admin/orders.php"
                       class="nav-admin-link <?= strpos($_SERVER['PHP_SELF'], '/admin/') !== false ? 'active' : '' ?>">
            ...
            <div class="header-actions">
                <?php if (isLoggedIn()): ?>
                    ...
                    <!-- Kushal: badge only shows if cart has items -->
                    ...
                    <!-- User dropdown menu -->
                    <!-- Kelsang: avatar shows first letter of first name -->
                    <!-- Rojal: dropdown toggle handled in main.js -->
                ...
                <?php else: ?>
                    <!-- Alok: login/register buttons when not signed in -->
            ...
            <!-- Mobile menu button -->
            <!-- Rojal: toggle handled in main.js -->
        ...
        <!-- Mobile nav -->
        <!-- Kelsang: simpler list version for small screens -->
